...
ca_cert = "/vagrant/keys_COMPROMISED/ca-chain.cert.pem"
#set to true to skip TLS and comment the line above
...
sudo echo "auth requisite pam_exec.so quiet expose_authtok log=/tmp/vaultssh.log /usr/local/bin/vault-ssh-helper -config=/etc/vault-ssh-helper.d/config.hcl" >> ${PAM_SSHD}
